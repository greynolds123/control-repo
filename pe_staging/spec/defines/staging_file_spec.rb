require 'spec_helper'
describe 'pe_staging::file', :type => :define do
  ...
  describe 'when deploying via puppet' do
    ...
    it {
      ...
      should_not contain_exec('/opt/puppetlabs/server/data/staging/spec/sample.tar.gz')
    }
  end
  ...
  describe 'when deploying via http' do
    ...
    it {
      should contain_file('/opt/puppetlabs/server/data/staging')
      should contain_exec('/opt/puppetlabs/server/data/staging/spec/sample.tar.gz').with( {
        :command => 'curl  -f -L -o sample.tar.gz http://webserver/sample.tar.gz',
        :path        => '/usr/local/bin:/usr/bin:/bin',
        :environment => nil,
        :cwd         => '/opt/puppetlabs/server/data/staging/spec',
        :creates     => '/opt/puppetlabs/server/data/staging/spec/sample.tar.gz',
  ...
  describe 'when deploying via https' do
     ...
     it { should contain_file('/opt/puppetlabs/server/data/staging') }
     it { should contain_exec('/opt/puppetlabs/server/data/staging/spec/sample.tar.gz').with( {
