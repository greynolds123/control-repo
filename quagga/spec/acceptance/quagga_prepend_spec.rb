...
describe 'quagga class prepend router' do
  ...
  context 'basic' do
    ...
    describe command("vtysh -c \'show ip bgp neighbors #{router2_ip} advertised-routes\'") do
      its(:stdout) { should match(/#{ipv4_network}\s+#{router1_ip}\s+0\s+32768\s+#{router1_asn}\s#{router1_asn}\s#{router1_asn}\si/) }
    end
    ...
    describe command("vtysh -c \'show ipv6 bgp neighbors #{router2_ip6} advertised-routes\'") do
      its(:stdout) { should match(/#{ipv6_network}\s+#{router1_ip6}\s+0\s+32768\s+#{router1_asn}\s#{router1_asn}\s#{router1_asn}\si/) }
    end
  end
end
