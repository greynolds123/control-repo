...
describe 'quagga failover server' do
  ...
  context 'basic' do
    ...
    describe command("vtysh -c \'show ip bgp neighbors #{router2_ip} received-routes\'") do
      its(:stdout) { should_not match(/#{ipv4_network}\s+#{router2_ip}\s+\d+\s+\d+\s+#{router2_asn}\s+i/) }
      its(:stdout) { should match(/#{ipv4_failsafe_network}\s+#{router2_ip}\s+\d+\s+\d+\s+#{router2_asn}\s+i/) }
    end
    ...
    describe command("vtysh -c \'show ipv6 bgp neighbors #{router2_ip6} received-routes\'") do
      its(:stdout) { should_not match(/#{ipv6_network}\s+#{router2_ip6}\s+\d+\s+\d+\s+#{router2_asn}\s+i/) }
      its(:stdout) { should match(/#{ipv6_failsafe_network}\s+#{router2_ip6}\s+\d+\s+\d+\s+#{router2_asn}\s+i/) }
    end
  end
end
