...
describe 'quagga class multi peers' do
  ...
  context 'basic' do
    ...
    describe command('vtysh -c \'show ip bgp sum\'') do
      ...
      its(:stdout) { should match(/#{router3_ip}\s+4\s+#{router3_asn}/) }
    end
    ...
    describe command("vtysh -c \'show ip bgp neighbors #{router2_ip} advertised-routes\'") do
      its(:stdout) { should_not match(/#{ipv4_network}\s+#{router1_ip}\s+\d+\s+\d+\s+i/) }
    end
    ...
    describe command("vtysh -c \'show ip bgp neighbors #{router2_ip} received-routes\'") do
        its(:stdout) { should_not match(/#{ipv4_network}\s+#{router2_ip}\s+\d+\s+\d+\s+#{router2_asn}\si/) }
    end
    describe command("vtysh -c \'show ip bgp neighbors #{router3_ip} received-routes\'") do
        its(:stdout) { should match(/#{ipv4_network}\s+#{router3_ip}\s+\d+\s+\d+\s+#{router3_asn}\si/) }
    end
    ...
    describe command("vtysh -c \'show ipv6 bgp neighbors #{router2_ip6} advertised-routes\'") do
      its(:stdout) { should_not match(/#{ipv6_network}\s+#{router1_ip6}\s+0\s+32768\s+i/) }
    end
    ...
    describe command("vtysh -c \'show ipv6 bgp neighbors #{router2_ip6} received-routes\'") do
        its(:stdout) { should match(/#{ipv6_network}\s+#{router2_ip6}\s+\d+\s+\d+\s+#{router2_asn}\si/) }
    end
    describe command("vtysh -c \'show ipv6 bgp neighbors #{router3_ip6} received-routes\'") do
        its(:stdout) { should_not match(/#{ipv6_network}\s+#{router3_ip6}\s+\d+\s+\d+\s+#{router3_asn}\si/) }
    end
  end
end
