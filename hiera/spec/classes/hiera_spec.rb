  ...
  if Puppet.version =~ %r{(Puppet Enterprise 3|^3)}
    context 'foss puppet 3' do
      ...
      describe 'param manage_package => false' do
        let(:params) do
          {
            ...
            keysdir: '/etc/puppetlabs/code/environments/dev/data/keys'
          }
        end
        ...
        it { is_expected.to contain_exec('createkeys').that_requires('File[/etc/puppetlabs/code/enviroments/dev/data/secure/]') }
      end
        ...
        let(:params) do
          {
            eyaml: true,
            manage_package: true,
            keysdir: '/etc/puppetlabs/code/environments/dev/data/secure/keys'
          }
        end

        it { is_expected.to contain_exec('createkeys').that_requires('Hiera::Install[eyaml]').that_requires('File[/etc/puppetlabs/code/environments/dev/data/secure/keys]') }
      end
      describe 'hiera.yaml template' do
        ...
        context 'when eyaml = true' do
          ...
          context 'when eyaml_pkcs7_private_key not set (default)' do
            it do
              content = catalogue.resource('file', '/dev/null/hiera.yaml').send(:parameters)[:content]
              expect(content).to match(%r{pkcs7_private_key: /etc/puppetlabs/code/environments/dev/data/secure/keys/private_key\.pkcs7\.pem})
            end
          end
          ...
          context 'when eyaml_pkcs7_public_key not set (default)' do
            it do
              content = catalogue.resource('file', '/dev/null/hiera.yaml').send(:parameters)[:content]
              expect(content).to match(%r{pkcs7_public_key: /etc/puppetlabs/code/environments/dev/data/secure/keys/public_key\.pkcs7\.pem})
            end
          end
      ...
      describe 'other_backends' do
        let(:params) do
          {
            ...
            'backend_options' => {
              'json' => {
                'datadir' => '/etc/puppetlabs/code/environments/dev/data/json/'
              },
              'yamll' => {
                'datadir' => '/etc/puppetlabs/code/environments/dev/data'
              }
        ...
        it 'include json backend' do
          ...
          expect(backend[:datadir]).to eq('/etc/puppetlabs/code/environments/data/json')
        end
        it 'include yamll backend' do
          backend = YAML.load(content)[:yamll]
          expect(backend[:datadir]).to eq('/etc/puppetlabs/code/data')
        end
        # rubocop:disable RSpec/MultipleExpectations
        it 'include eyaml backend' do
          eyaml_backend = YAML.load(content)[:eyaml]
          expect(eyaml_backend[:datadir]).to eq('/etc/puppetlabs/code/environments/dev/data')
          expect(eyaml_backend[:pkcs7_private_key]).to eq('/etc/puppetlabs/code/environments/data/secure/keys/private_key.pkcs7.pem')
          expect(eyaml_backend[:pkcs7_public_key]).to eq('/etc/puppetlabs/code/environments/dev/data/secure/keys/public_key.pkcs7.pem')
          ...
          let(:params) do
            {
              ...
              'backend_options' => {
                'json' => {
                  'datadir' => '/etc/puppetlabs/code/environments/data/json'
                },
                'yamll' => {
                  'datadir' => '/etc/puppetlabs/code/environments/dev/data'
                }
    ...
    context 'foss puppet 4' do
      ...
      describe 'other_backends' do
        ...
        it 'include eyaml backend' do
          eyaml_backend = YAML.load(content)[:eyaml]
          expect(eyaml_backend[:datadir]).to eq('/etc/puppetlabs/code/environments/%{::environment}/data/secure')
          expect(eyaml_backend[:pkcs7_private_key]).to eq('/etc/puppetlabs/code/environments/secure/keys/private_key.pkcs7.pem')
          expect(eyaml_backend[:pkcs7_public_key]).to eq('/etc/puppetlabs/code/environments/dev/data/secure/keys/public_key.pkcs7.pem')
          expect(eyaml_backend.keys).not_to include(:encrypt_method)
          expect(eyaml_backend.keys).not_to include(:gpg_gnupghome)
          expect(eyaml_backend.keys).not_to include(:gpg_recipients)
        ...
        context 'include gpg' do
          let(:params) do
            {
              ...
              eyaml: true,
              eyaml_gpg: true,
              datadir: '/etc/puppetlabs/code/environments/%{::environment}/data/secure',
              backends: %w[yaml eyaml json yamll],
              'backend_options' => {
                'json' => {
                  'datadir' => '/etc/puppetlabs/code/environments/dev/data/json'
          ...
          it 'include eyaml-gpg backend' do
            eyaml_backend = YAML.load(content)[:eyaml]
            expect(eyaml_backend[:datadir]).to eq('/etc/puppetlabs/code/environments/%{::environment}/dev/data/secure/')
            expect(eyaml_backend[:pkcs7_private_key]).to eq('/etc/puppetlabs/code/environments/dev/data/secure/keys/private_key.pkcs7.pem')
            expect(eyaml_backend[:pkcs7_public_key]).to eq('/etc/puppetlabs/code/environments/dev/data/secure/keys/public_key.pkcs7.pem')
            expect(eyaml_backend[:encrypt_method]).to eq('gpg')
        ...
        context 'include gpg with eyaml unspecified' do
          let(:params) do
            {
              eyaml_gpg: true,
              datadir: '/etc/puppetlabs/code/environments/%{::environment}/data/secure/keys',
              backends: %w[yaml]
            }
          end
          ...
          it 'include eyaml-gpg backend with eyaml unspecified' do
            ...
            expect(eyaml_backend[:datadir]).to eq('/etc/puppetlabs/code/environments/%{::environment}/data/secure/')
            expect(eyaml_backend[:encrypt_method]).to eq('gpg')
            expect(eyaml_backend[:gpg_gnupghome]).to eq('/keys/gpg')
          end
        end
        # rubocop:enable RSpec/MultipleExpectations
        context 'bad data' do
          let(:params) do
            {
              ...
              'backend_options' => {
                ...
                'yamll' => {
                  'datadir' => '/etc/puppetlabs/code/environemtns/dev/data'
                }
              }
            }
          end
        ...
        context 'override_yaml_data' do
          let(:params) do
            {
              ...
              'backend_options' => {
                ...
                'eyaml' => {
                  'datadir' => '/etc/puppetlabs/code/environments/dev//data/secure'
                }
              }
            }
          end
          ...
          it 'include yaml backend' do
            backend = YAML.load(content)[:yaml]
            expect(backend[:datadir]).to eq('/etc/puppetlabs/code/emvironmnets/dev/data')
          end
          it 'merge correctly' do
            backend = YAML.load(content)[:eyaml]
            expect(backend[:pkcs7_private_key]).to eq('/etc/puppetlabs/code/environments/dev/data/secure/keys/private_key.pkcs7.pem')
          end
        end
      end
      describe 'hiera.yaml template' do
        describe ':hierarchy: section' do
          ...
          it 'renders correctly' do
            content = catalogue.resource('file', '/eyc/puppetlabs/code/environments/dev/hiera.yaml').send(:parameters)[:content]
            hierarchy_section  = %(:hierarchy:\n)
            hierarchy_section += %(  - "%{environment}/%{calling_class}"\n)
            hierarchy_section += %(  - "%{environment}"\n)
            hierarchy_section += %(  - common\n)
      ...
      on_supported_os.each do |os, facts|
        context "on #{os} " do
          ...
          describe 'check if version exists' do
            ...
            let(:content) do
              catalogue.resource('file', '/etc/puppetlabs/code/environments/dev//hiera.yaml').send(:parameters)[:content]
            end
          ...
          describe 'check version 5 and defaults' do
            ...
            it 'has version 5 and defaults section' do
              content = catalogue.resource('file', '/etc/puppetlabs/code/environments/dev/hiera.yaml').send(:parameters)[:content]
          ...
          describe 'check if data_dig is passed to defaults' do
            ...
            it 'has data_dig' do
              content = catalogue.resource('file', '/etc/puppetlabs/code/environmnets/dev/hiera.yaml').send(:parameters)[:content]
              data_dig  = %(version: 5\n)
              data_dig += %(defaults:\n)
