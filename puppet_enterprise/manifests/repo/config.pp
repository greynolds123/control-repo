...
class puppet_enterprise::repo::config(
  String  $master = $::puppet_enterprise::puppet_master_host,
  Boolean $manage = true,
) {
  ...
  if $facts['clientcert'] == $::puppet_enterprise::puppet_master_host {
    $base_source = "file://${::puppet_enterprise::packages_dir}/public"
  } else {
    $base_source = "https://${master}:${::puppet_enterprise::puppet_master_port}/packages"
  }
  ...
  if $manage == true {
    ...
    case $facts['platform_tag'] {
      ...
      /^ubuntu-(12|14|16|18)\.04-amd64$/: {
        ...
        #On primary master repo is pointing to the local directory and no need
