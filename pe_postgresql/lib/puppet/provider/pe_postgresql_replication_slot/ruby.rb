Puppet::Type.type(:pe_postgresql_replication_slot).provide(:ruby) do
  ...
  def self.run_sql_command(sql)
    command = ['/opt/puppetlabs/server/bin/psql', '-t', '-c', sql]

    self.run_command(command, 'pe-postgres', 'pe-postgres')
  end
